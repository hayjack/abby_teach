...
def reconcile_student_hours(student_id, course_id):
    ...
    最终结果都只取决于当前的考勤状态。
    """
    from models import StudentCourse, ClassRecord, AttendanceRecord
...
def sync_leave_on_record_creation(class_record):
    ...
    logger.info(f'[考勤] 班级共有 {len(students)} 名学生')
    ...
    # 为所有学生重新计算课时（幂等操作，确保数据一致性）
    for student in students:
        reconcile_student_hours(student.id, course_id)
